...
function setup() {
  ...
  showButton.mousePressed(() => {
    ...
    elementKText = femSolver.getElementStiffnessText(highlightNode);
  });
...
function updateForceVectorText() {
  forceText = "Global Force Vector:\n";
  for (let i = 0; i < femSolver.F.length; i++) {
    forceText += `F[${i}] = ${femSolver.F[i].toFixed(2)} N\n`;
  }
}
...
function draw() {
  ...
  fill(240);
  rect(450, 150, 400, 200);
  fill(0);
  text(forceText, 455, 165);

  fill(240);
  rect(880, 150, 480, 220);
  fill(0);
  text("Element Stiffness Matrices:", 885, 165);
  text(elementKText, 885, 180);
}
...
class FEMSolver {
  ...
  drawDisplacementPlot() {
    ...
    const scaleX = 1000 / this.L, scaleY = 1e5;
    stroke(0);
    noFill();
    beginShape();
  ...
  getElementStiffnessText(nodeId) {
    if (nodeId < 0 || nodeId >= this.nodes.length) return "❌ Invalid node.";
    let txt = "";
    for (let e of this.elements) {
      if (e.isConnectedTo(nodeId)) {
        const k = e.getLocalKMatrix();
        txt += `\nElement ${e.id} [${e.n1.id}-${e.n2.id}]:\n`;
        txt += `[${k[0][0].toExponential(2)}  ${k[0][1].toExponential(2)}]\n`;
        txt += `[${k[1][0].toExponential(2)}  ${k[1][1].toExponential(2)}]\n`;
      }
    }
    return txt;
  }
